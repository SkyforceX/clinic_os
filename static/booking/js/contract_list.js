  ...
  document.querySelectorAll('.open-upload-modal-desktop, .open-upload-modal-mobile').forEach(button => {
    button.addEventListener('click', function (e) {
      e.preventDefault();
        ...
        const API_PATIENTS_JSON_URL = `ajax/patients_json/${companyId}/`;

        fetch(API_PATIENTS_JSON_URL, {
          method: 'GET',
          headers: {
        ...
        .then(async (res) => {
          ...
          // Nếu HTTP không OK, ném lỗi kèm đoạn trích body
          if (!res.ok) {
          ...
          // Nếu không phải JSON (thường là HTML trang login/lỗi)
      ...
      .then(data => {
        ...
        .catch(err => {
          console.error(err);
          document.getElementById('patientTableBody').innerHTML = `<tr><td colspan="4" class="text-danger text-center">${err}</td></tr>`;
