...
function updateDepositCalc() {
  ...
  const baseMale = ccData.maleCount;
  const baseFs   = ccData.femaleSingleCount;
  const baseFf   = ccData.femaleFamilyCount;

  const unitMale = baseMale > 0 ? Math.round(ccData.subtotalMale / baseMale) : 0;
  const unitFs   = baseFs > 0 ? Math.round(ccData.subtotalFemaleSingle / baseFs) : 0;
  const unitFf   = baseFf > 0 ? Math.round(ccData.subtotalFemaleFamily / baseFf) : 0;

  const grand = (maleCount * unitMale) + (fsCount * unitFs) + (ffCount * unitFf);
