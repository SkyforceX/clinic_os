...
class MeetingCommitment(models.Model):
    ...
    # FK đến tasks.Task — dùng string reference để tránh circular import
    # Được set sau khi close_session() → create_tasks_from_session()
    ...
    @property
    def task_stage_display(self) -> str:
        """Hiển thị trạng thái task liên kết (nếu có)."""
        if not self.task_id:
            return ""
        try:
            return self.task.get_stage_display()
        except Exception:
            return ""
...
class MeetingSignature(models.Model):
    ...
    doc_hash   = models.CharField(max_length=64, verbose_name="SHA-256 hash biên bản")
    ...
    def compute_hash(cls, session) -> str:
        payload = {
            ...
            "dept_assignments": [
                {"id": a.pk, "department": a.department, "confirmed": a.confirmed}
