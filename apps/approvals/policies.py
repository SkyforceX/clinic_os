...
class ApprovalPolicy:
    ...
    def can_view_inbox(cls, user) -> bool:
        """Chỉ Executives (và superuser) xem được inbox."""
        return ContractPolicy.is_executive(user)

    @classmethod
    def can_approve(cls, user, ar) -> bool:
        """
        Executives, request đang PENDING, không tự duyệt chính mình
        (trừ superuser).
        """
    ...
    @classmethod
    def can_revoke_approved(cls, user, ar) -> bool:
        if not ContractPolicy.is_authenticated_actor(user):
            return False
        if not getattr(user, "is_superuser", False):
            return False
        return getattr(ar, "status", "") == "APPROVED"
