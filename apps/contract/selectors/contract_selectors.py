...
def contract_base_queryset():
    return HealthContract.objects.select_related("company", "created_by").all()
...
def get_contract_detail(*, user, contract_id):
    ...
    contract.blood_collection_rows = list(
        BloodCollectionInfo.objects.filter(contract_id=contract.id).order_by("collection_date", "id")
    )
    contract.schedule_rows = list(
        AppointmentSchedule.objects.filter(contract_id=contract.id).order_by("date", "shift")
    )
    return contract
