...
from apps.contract.services.package_snapshot import build_contract_package_snapshot_from_quotation
...
def _build_contract_service_line_from_quotation_line(
    *,
    ...
    price_type = (getattr(quotation_line, "price_type", "standard") or "standard").strip().lower()
    if price_type not in {"standard", "free", "gift"}:
        price_type = "standard"

    note = (getattr(quotation_line, "note", "") or "").strip()
    if not note:
        if price_type == "free":
            note = "Miễn phí"
    ...
    is_special_price = price_type in {"free", "gift"}
    ...
    return ContractServiceLine(
        ...
        for_male=male_count > 0 and checked_male and (is_special_price or price_male > 0),
        for_female_single=female_single_count > 0 and checked_female_single and (is_special_price or price_female_single > 0),
        for_female_family=female_family_count > 0 and checked_female_family and (is_special_price or price_female_family > 0),
        ...
        discount_male_pct=getattr(quotation_line, "discount_male_pct", 0) or 0,
        discount_fs_pct=getattr(quotation_line, "discount_fs_pct", 0) or 0,
        discount_ff_pct=getattr(quotation_line, "discount_ff_pct", 0) or 0,
...
def _build_contract_service_line_from_payload_item(
    *,
    ...
    price_type = (item.get("price_type") or "standard").strip().lower()
    if price_type not in {"standard", "free", "gift"}:
        price_type = "standard"
    ...
    return ContractServiceLine(
        ...
        price_female_single=_line_decimal_or_none(price_female_single),
        price_female_family=_line_decimal_or_none(price_female_family),
        discount_male_pct=item.get("discount_male_pct") or 0,
        discount_fs_pct=item.get("discount_fs_pct") or 0,
        discount_ff_pct=item.get("discount_ff_pct") or 0,
        price_type=price_type,
        note=note,
        display_order=order,
    )
...
def _parse_contract_form_payload(request, *, quotation_obj):
    # Multi-package: lấy count từ columns_json của từng gói
    # Legacy single-package: lấy từ field cũ trên QuotationDraft
    packages = list(quotation_obj.packages.all())
    if packages:
        male_count = 0
        female_single_count = 0
        female_family_count = 0
        employee_count = 0
        for pkg in packages:
            for col in (pkg.columns_json or []):
                count = parse_int(col.get("count"), 0)
                key = str(col.get("key") or "")
                employee_count += count
                if key == "male":
                    male_count += count
                elif key == "female_single":
                    female_single_count += count
                elif key == "female_family":
                    female_family_count += count
    else:
        male_count = parse_int(getattr(quotation_obj, "male_count", 0), 0)
        female_single_count = parse_int(getattr(quotation_obj, "female_single_count", 0), 0)
        female_family_count = parse_int(getattr(quotation_obj, "female_family_count", 0), 0)
        employee_count = male_count + female_single_count + female_family_count

    if employee_count <= 0:
        raise ValidationError(
            {
                "quotation_id": ["Báo giá gốc chưa có số lượng nhân sự hợp lệ để tạo hợp đồng."]
            }
        )
...
def create_corporate_contract_from_request(request):
    ...
    if quotation_obj.status != QuotationStatus.APPROVED:
        raise ValidationError({"quotation_id": ["Chỉ được tạo hợp đồng từ báo giá đã duyệt."]})
    ...
    if not latest_quotation or latest_quotation.id != quotation_obj.id:
        raise ValidationError(
            {"quotation_id": ["Chỉ được tạo hợp đồng từ báo giá đã duyệt mới nhất của công ty này."]}
        )
    ...
    package_snapshot_json = build_contract_package_snapshot_from_quotation(
        quotation_obj,
        participant_counts={
    ...
    profile = CorporateContractProfile.objects.create(
        ...
        package_snapshot_json=package_snapshot_json,
        quotation_extra_content_snapshot=quotation_obj.extra_content or None,
...
# ======== xóa hàm vì không dùng nút chốt  =========
# @transaction.atomic
...
def build_quote_context(contract):
    ...
    # Use pre-computed lines from payload — they already contain male_mark,
    # female_single_mark, female_family_mark, display_unit_price, stt, group_name, etc.
    details = contract_payload["lines"]
    
    return {
        "contract": contract,
        "profile": profile,
        "details": details,
        "packages": contract_payload.get("packages", []),
        "multi_package": contract_payload.get("multi_package", False),
