...
function setSubmittingState(isBusy) {
  ...
  if (submitBtn) {
    const defaultHtml = submitBtn.dataset.defaultHtml || 'Lưu hợp đồng';
...
function loadQuotationData(qid) {
  ...
  if (!qid) {
    ...
    if (ffDisplay) ffDisplay.value = 0;
  ...
  if (ffDisplay) ffDisplay.value = opt.dataset.ff || 0;

  selectedGrand = parseInt(opt.dataset.grand || 0, 10);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('input', function (e) {
    ...
    if (name) {
      const errorBox = document.querySelector(`.cc-field-error[data-error-for="${CSS.escape(name)}"]`);
  ...
  form.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        if (errorBox) {
          errorBox.textContent = data.message || 'Không thể lưu hợp đồng.';
      ...
      if (errorBox) {
        errorBox.textContent = 'Đã xảy ra lỗi khi gửi dữ liệu.';
        errorBox.style.display = 'block';
      }
    } finally {
