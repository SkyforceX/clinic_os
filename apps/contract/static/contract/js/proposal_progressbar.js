(function () {
  const form = document.getElementById("issueQuotationForm");
  const submitBtn = document.getElementById("issueQuotationBtn");
  ...
  const milestones = [
    { until: 18, step: 1, text: "Đang chuẩn bị dữ liệu báo giá..." },
    // { until: 48, step: 2, text: "Đang tạo file DOCX..." },
    { until: 82, step: 2, text: "Đang chuyển đổi sang PDF..." },
    { until: 95, step: 3, text: "Đang hoàn tất phát hành tài liệu..." },
  ];
  ...
  function renderProgress(value) {
    ...
    if (value >= 95) {
      const last = steps.find(el => Number(el.dataset.step || "0") === 4);
      if (last) {
  ...
  function disableActionArea() {
    ...
    submitBtn.classList.add("btn-loading-disabled");
    submitBtn.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Đang phát hành...';
